...
class Biblioteca:
    ...
    def buscar_ieee(query):
        with sync_playwright() as p:
            ...
            articulos = []
            for r in resultados[:3]:  # Limitar a 3 artículos para pruebas
                try:
                    r.scroll_into_view_if_needed()
                    r.click(force=True)  # Forzar el clic
                    pagina.wait_for_timeout(5000)
    ...
    @staticmethod
    def extraer_cita_ieee(pagina):
        try:
            with pagina.expect_download() as descarga:
                # Buscar y hacer scroll hasta el botón "Cite This"
                ...
                # Esperar a que aparezca el menú de citas
            ...
            archivo_descargado = descarga.value.path()
            return f"Cita descargada correctamente en: {archivo_descargado}"
        except Exception as e:
